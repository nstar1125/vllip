...
class clip_backbone(nn.Module):
    ...
    def __init__(self, model_name = 'RN50', visual_projs_path = './pretrain/'):
        ...
        self.model.visual.conv1 = nn.Conv2d(in_channels=9, out_channels=32, kernel_size=3, stride=2, padding=1, bias=False)
...
class CLIPViTFM(nn.Module):
    ...
    def forward(self, image, pred_masks,  masking_block=None, masking_type='token_masking'): #시작
        ...
        x = x.reshape(x.shape[0], x.shape[1], -1) # [*, 768, 49]

        x = x.permute(0, 2, 1)  # [*, 49, 768]
        x = torch.cat([vit.class_embedding.to(x.dtype) + torch.zeros(x.shape[0], 1, x.shape[-1],
                                                                     dtype=x.dtype, device=x.device), x], dim=1)  # shape = [*, grid ** 2 + 1, width]
        # [*, 49, 768] + [*, 1, 768] = [*, 49+1, 768]

        x = x + vit.positional_embedding.to(x.dtype) # [*, 49+1, 768]
        # x = x + self.original_pos_embedding
        x = vit.ln_pre(x) # [*, 49+1, 768]
        ...
        #L, N, D = 49, *, 768
        size = int(np.sqrt(L)) #7
        assert size * size == L 

        pred_masks = torch.mean(pred_masks, dim=1, keepdim=True)
        ...
        if masking_type == 'token_masking':
            for block_idx, resblock in enumerate(vit.transformer.resblocks):
                if block_idx >= masking_block: #Masking 작업 수행
                    cls = x[:1,:,:] #[1, *, 768]
                    x = x[1:,:,:] # [49, *, 768]
                    x = x.permute(1,2,0) # [*, 768, 49]
                    x = x.view(N, D, size, size).contiguous() # [1, 768, 7, 7] / [*, 768, 7, 7]
                    x = torch.mul(x, pred_masks.expand(-1, x.size()[1], -1, -1)) # [*, 768, 7, 7] x [*, None, 7, 7] -> [*, 768, 7, 7]
                    N = x.size(0) # N = *            
                    x = x.view(N, D, L).contiguous() # [*, 768, 7, 7] -> [*, 768, 49]
                    x = x.permute(2,0,1) # [49, *, 768]
                    x = torch.cat([cls.expand(-1,N,-1), x], dim=0) # [49+1, *, 768]
                    x = resblock(x) # [49+1, *, 768]

                    if block_idx == self.last_layer:
                        
                        x = x.permute(1, 0, 2) # [*, 50, 768]
                        
                        x = self.model.visual.ln_post(x[:, 0, :]) # [*, 768]

                        if self.model.visual.proj is not None:
                            x = x @ self.model.visual.proj # 768->512
                        
                        x = self.final_conv(x) # 추가
